...
class ContentTestCase(BaseTestCase):
    ...
    def test_update_simple(self):
        ...
        new_klnk = self.bkend.update_kilink(klnk.linkode_id, child_content, "type2")
...
class DataRetrievalTestCase(BaseTestCase):
    ...
    def test_getkilink_id_bad(self):
        ...
        self.assertRaises(KilinkNotFoundError, self.bkend.get_kilink, "bad linkode_id")
    ...
    def test_getkilink_tree_ok(self):
        ...
        self.assertEqual(tree[1].text_type, PLAIN_TEXT)
        ...
        self.assertEqual(tree[1].parent, tree[0].linkode_id)  # 2nd is child of root
        self.assertEqual(tree[2].parent, tree[1].linkode_id)  # 3rd is child of 2nd
        self.assertEqual(tree[3].parent, tree[0].linkode_id)  # 4th is child of root

    def test_getkilink_tree_bad(self):
        """Get all the info for a kilink that does not exist"""
        self.assertRaises(KilinkNotFoundError, self.bkend._get_kilink_tree, 'linkode_id')
    ...
    def test_findroot_tree_bad(self):
        ...
        self.assertRaises(KilinkNotFoundError, self.bkend._get_root_node, 'linkode_id')
